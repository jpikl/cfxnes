...
import {closeAudioContext} from '../../src/audio/context';
...
describe('audio/Audio', () => {
  ...
  for (const channel of channels) {
    ...
    it(`has ${volume} ${channel} volume by default`, () => {
      expect(audio.getVolume(channel)).to.be.equal(volume);
    });
  }
  ...
  for (const channel of channels) {
    it(`throws error when setting invalid ${channel} volume`, () => {
      expect(() => audio.setVolume(channel)).to.throw('Invalid audio volume');
      expect(() => audio.setVolume(channel, -0.1)).to.throw('Invalid audio volume');
      expect(() => audio.setVolume(channel, 1.1)).to.throw('Invalid audio volume');
    });
  }
