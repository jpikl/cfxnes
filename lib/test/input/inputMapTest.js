...
describe('input/InputMap', () => {
  ...
  it('throws error when calling get with invalid arguments', () => {
    ...
    expect(() => map.get('a', 'b')).to.throw('Invalid arguments');
  });
  ...
  it('throws error when calling set with invalid arguments', () => {
    expect(() => map.set()).to.throw('Invalid arguments');
    expect(() => map.set('a', 'b', 'c')).to.throw('Invalid arguments');
    expect(() => map.set('a')).to.throw('Invalid mapping');
    expect(() => map.set(1, 'keyboard.x')).to.throw('Invalid device input');
    expect(() => map.set('1.joypad.a', 1)).to.throw('Invalid source input(s)');
  });
  ...
  it('throws error when unmapping invalid inputs', () => {
    expect(() => map.delete(1)).to.throw('Invalid input');
  });
