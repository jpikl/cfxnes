...
describe('input/InputRouter', () => {
  let nes, mapper, devices, video, router;
  ...
  it('should not route mouse inputs when there is no video output', () => {
    video.getOutput = () => null;
    devices.set(1, 'zapper');
    mapper.map('1.zapper.trigger', 'mouse.left');
    let result;
  ...
  it('should route only mouse clicks pointing to video output', () => {
    devices.set(1, 'zapper');
    mapper.map('1.zapper.trigger', 'mouse.left');
    let result;
