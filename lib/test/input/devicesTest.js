...
describe('input/Devices', () => {
  ...
  it('throws error when setting invalid device', () => {
    expect(() => devices.set(1)).to.throw('Invalid device');
    expect(() => devices.set(1, 'x')).to.throw('Invalid device');
  });

  it('throws error when setting device on invalid port', () => {
    expect(() => devices.set()).to.throw('Invalid port');
    expect(() => devices.set('x', 'joypad')).to.throw('Invalid port');
    expect(() => devices.set(0, 'joypad')).to.throw('Invalid port');
    expect(() => devices.set(3, 'joypad')).to.throw('Invalid port');
  });

  it('throws error when getting device on invalid port', () => {
    expect(() => devices.get()).to.throw('Invalid port');
    expect(() => devices.get('x')).to.throw('Invalid port');
    expect(() => devices.get(0)).to.throw('Invalid port');
    expect(() => devices.get(3)).to.throw('Invalid port');
  });
