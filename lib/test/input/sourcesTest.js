...
import {dispatchKeyboardEvent, dispatchMouseEvent} from './events';
...
describe('input/Sources', () => {
  ...
  it('throws error when callback is not provided for recording', () => {
    expect(() => sources.recordInput()).to.throw('Invalid record input callback');
    expect(() => sources.recordInput('x')).to.throw('Invalid record input callback');
  });
