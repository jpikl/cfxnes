...
describe('video/Video (no canvas)', () => {
  ...
  it('sets output provided as canvas ID', () => {
    const canvas = document.createElement('canvas');
    canvas.id = 'unique-id';
    document.body.appendChild(canvas);
    try {
      video.setOutput(canvas.id);
      expect(video.getOutput()).to.be.equal(canvas);
    } finally {
  ...
  it('throws error for invalid canvas ID', () => {
    expect(() => video.setOutput('invalid-id')).to.throw('Unable to find canvas with specified ID');
  });

  it('throws error for ID that does not belong to a canvas', () => {
    const element = document.createElement('p');
    element.id = 'unique-id';
    document.body.appendChild(element);
    try {
      expect(() => video.setOutput(element.id)).to.throw('Element with specified ID is not a canvas');
    } finally {
      document.body.removeChild(element);
    }
  });

  it('throws error when provided invalid canvas ID', () => {
    expect(() => video.setOutput('invalid-id')).to.throw('Unable to find element by ID');
  });
