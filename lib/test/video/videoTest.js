...
describe('video/Video (no canvas)', () => {
  ...
  it('has "nearest" filter by default', () => {
    expect(video.getFilter()).to.be.equal('nearest');
  });

  it('changes filter', () => {
    video.setFilter('linear');
    expect(video.getFilter()).to.be.equal('linear');
    video.setFilter('nearest');
    expect(video.getFilter()).to.be.equal('nearest');
  });

  it('throws error when setting invalid filter', () => {
    expect(() => video.setFilter()).to.throw('Invalid video filter');
    expect(() => video.setFilter('x')).to.throw('Invalid video filter');
  });
