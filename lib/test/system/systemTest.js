...
describe('system/System', () => {
  ...
  asyncIt('does not render/draw frames while running with no video output', 50, () => {
    system.start();
  }, () => {
    expect(video.framesRendered).to.be.equal(0);
    expect(video.framesDrawn).to.be.equal(0);
  });

  asyncIt('renders/draws frames while running with video output set', 50, () => {
  ...
  asyncIt('renders/draws zero frames per step when no video output is set', 50, () => {
    system.step();
  }, () => {
    expect(video.framesRendered).to.be.equal(0);
    expect(video.framesDrawn).to.be.equal(0);
  ...
  asyncIt('renders/draws one frame per step when video output is set', 50, () => {
    video.output = document.createElement('canvas');
    system.step();
  }, () => {
  ...
  it('throws error when setting invalid region', () => {
    expect(() => system.setRegion()).to.throw('Invalid region');
    expect(() => system.setRegion('x')).to.throw('Invalid region');
  });
  ...
  it('throws error when setting invalid speed', () => {
    expect(() => system.setSpeed()).to.throw('Invalid speed');
    expect(() => system.setSpeed('x')).to.throw('Invalid speed');
    expect(() => system.setSpeed(0)).to.throw('Invalid speed');
    expect(() => system.setSpeed(-1)).to.throw('Invalid speed');
  });
