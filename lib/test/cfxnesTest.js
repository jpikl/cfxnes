...
export default function test(name, cfxnes) {
  describe(name, () => {
    ...
    it('throws error when called with invalid argument', () => {
      expect(() => cfxnes('x')).to.throw('Invalid parameter');
    });
    ...
    describe('nvram', () => {
      ...
      it('has data when present', () => {
        ...
        return rom.load('roms/M1_P128K_C128K_S8K.nes').then(() => {
          ...
          expect(nvram.data).to.have.lengthOf(0x2000);
        });
      });
    ...
    describe('video', () => {
      ...
      it('provides maximum scale', () => {
        expect(video.maxScale).to.be.at.least(1);
      });

      it('changes smoothing', () => {
        video.smoothing = true;
        expect(video.smoothing).to.be.true;
      });
      ...
      it('uses default parameters', () => {
        ...
        expect(video.smoothing).to.be.false;
    ...
    describe('inputs', () => {
      ...
      beforeEach(function() {
        if (canInit) {
          inputs = cfxnes().inputs;
        } else {
          this.skip();
        }
      });

      it('maps/unmaps/returns inputs', () => {
        inputs.unmap();
        expect(inputs.get('1.joypad.a')).to.deep.equal([]);
        inputs.map('1.joypad.a', ['keyboard.x', 'keyboard.y']);
        expect(inputs.get('1.joypad.a')).to.deep.equal(['keyboard.x', 'keyboard.y']);
        inputs.unmap('keyboard.x');
        expect(inputs.get('1.joypad.a')).to.deep.equal(['keyboard.y']);
        inputs.unmap();
        expect(inputs.get('1.joypad.a')).to.deep.equal([]);
      });
      ...
      it('uses default parameters', () => {
        expect(inputs.get('1.joypad.b')).to.deep.equal(['keyboard.y', 'keyboard.z']);
      });
      ...
      it('uses provided parameters', () => {
        ...
        inputs = nes.inputs;
        expect(inputs.get('1.joypad.b')).to.deep.equal(['keyboard.a']);
      });
    });
    ...
    describe('options', () => {
      ...
      it('gets', () => {
        expect(options.get().region).to.be.equal('auto');
      });

      it('sets', () => {
        options.set({region: 'ntsc'});
        expect(nes.region).to.be.equal('ntsc');
      });

      it('resets', () => {
        nes.region = 'ntsc';
        options.reset('region');
        expect(nes.region).to.be.equal('auto');
      });
