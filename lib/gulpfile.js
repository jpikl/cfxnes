...
const filesChoices = ['all', 'mod', 'lib'];
...
const argv = yargs
  ...
  .option('b', {
    ...
    alias: 'browsers',
    type: 'string',
    default: 'Chrome,Firefox,Edge,Opera',
  })
  .option('e', {
    desc: 'IP address and port "ip[:port]" of host running webdriver server for MS Edge',
  ...
  .example('gulp test -f mod', 'Run test for source modules')
  .example('gulp test -f lib', 'Run tests for build library')
...
gulp.task('karma', done => {
  ...
  if (argv.files === 'mod') {
    ...
    files = moduleFiles.concat(libraryFiles);
  }
