import {log, describeValue, isPaletteName, createPalette, DEFAULT_PALETTE, VIDEO_WIDTH, VIDEO_HEIGHT} from '../../../core';
...
export default class Video {
  ...
  setOutput(canvas) {
    if (canvas !== null && !(canvas instanceof HTMLCanvasElement)) {
      throw new Error('Invalid video output: ' + describeValue(canvas));
    }
  ...
  setRenderer(name) {
    if (!isRendererName(name)) {
      throw new Error('Invalid video renderer: ' + describeValue(name));
    }
  ...
  setPalette(name) {
    if (!isPaletteName(name)) {
      throw new Error('Invalid video palette: ' + describeValue(name));
    }
  ...
  setScale(scale) {
    if (typeof scale !== 'number' || scale <= 0) {
      throw new Error('Invalid video scale: ' + describeValue(scale));
    }
  ...
  setFilter(filter) {
    if (!isFilter(filter)) {
      throw new Error('Invalid video filter: ' + describeValue(filter));
    }
  ...
  setDebug(debug) {
    if (typeof debug !== 'boolean') {
      throw new Error('Invalid video debug: ' + describeValue(debug));
    }
  ...
  setFullscreenType(type) {
    if (!isStyleName(type)) {
      throw new Error('Invalid fullscreen type: ' + describeValue(type));
    }
