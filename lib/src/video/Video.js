...
export default class Video {
  ...
  setRenderer(name) {
    if (this.rendererName !== name) {
      assert(this.canvas == null, 'Unable to change renderer, canvas context is already initialized');
  ...
  setDebug(enabled) {
    if (this.debug !== enabled) {
      log.info(`Setting video debug to ${enabled ? 'on' : 'off'}`);
      this.debug = enabled;
      if (this.canvas) {
        this.updateRenderer();
        this.updateSize();
  ...
  enterFullscreen() {
    assert(this.canvas != null, 'No canvas was set as video output');
