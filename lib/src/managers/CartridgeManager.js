    ...
    return new Promise((resolve, reject) => {
      ...
      this.removeCartridge().then(() => {
        ...
        return this.persistenceManager.loadCartridgeData().catch(error => {
          logger.error('Error during load of cartridge data', error); // Do not propagate loadCartridgeData errors!
        });
      }).then(() => {
    ...
    if (this.isCartridgeInserted()) {
      return new Promise((resolve, reject) => {
        this.persistenceManager.saveCartridgeData().then(() => {
          ...
          logger.error('Error during save of cartridge data', error); // Do not propagate saveCartridgeData errors!
        }).catch(reject);
      });
    }
