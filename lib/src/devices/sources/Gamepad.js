...
export default class Gamepad {
  ...
  onAxisChange(gamepad, axisIndex, oldValue, newValue) {
    if (oldValue) {
      var input = this.createAxisName(gamepad, axisIndex, oldValue);
      this.processInput(gamepad, input, false);
    }
    if (newValue) {
      var input = this.createAxisName(gamepad, axisIndex, newValue);
      this.processInput(gamepad, input, true);
    }
  }

  onButtonChange(gamepad, buttonIndex, newValue) {
    var input = this.createButtonName(gamepad, buttonIndex);
  ...
  createAxisName(gamepad, axisIndex, value) {
    ...
    if (gamepad.mapping === 'standard') {
      return axisAliases[axisIndex] + direction; // Standard name
    }
  ...
  createButtonName(gamepad, buttonIndex) {
    if (gamepad.mapping === 'standard') {
      return buttonAliases[buttonIndex]; // Standard name
    }
