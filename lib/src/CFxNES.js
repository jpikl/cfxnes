...
export default class CFxNES {
  ...
  constructor(params = {}) {
    ...
    if ('videoOutput' in params) {
      video.setOutput(params['videoOutput']);
    }

    Object.assign(this, {system, video, audio, devices, sources, inputMapper, rom, nvram, options});

    if ('romSource' in params) {
      rom.load(params['romSource'])
        .then(() => system.start())
        .catch(log.error);
    }
  }
  ...
  ['isROMLoaded']() {
    return this.rom.isLoaded();
  }

  ['getNVRAM']() {
    return this.nvram.access();
  }
...
CFxNES['getLogLevel'] = log.getLevel;

CFxNES['close'] = function() {
