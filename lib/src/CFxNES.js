...
export default class CFxNES {
  ...
  constructor(config = {}) {
    ...
    logger.setLevel(logLevelAliases[config['logLevel']] || LogLevel.WARN);
  ...
  loadOnStart(config) {
    if (config['loadOnStart']) {
      return this['loadConfiguration']().catch((error) => {
        logger.error('Error during load of configuration', error);
      });
    }
