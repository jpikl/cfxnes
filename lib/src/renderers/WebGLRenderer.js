import {BLACK_COLOR} from '../../../core/src/constants';
import {roundUpToPowerOf2} from '../../../core/src/utils';
...
export default class WebGLRenderer {
  ...
  getContext(id) {
    try {
      log.info(`Trying to get WebGL context "${id}"`);
      return this.canvas.getContext(id);
    } catch (error) {
      log.warn(`Error when getting WebGL context "${id}": ${error}`);
      return null;
    }
  }
  ...
  createFrameData(width, height) {
    ...
    return new Uint32Array(buffer).fill(BLACK_COLOR);
  }
