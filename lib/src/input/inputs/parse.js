...
export default function parseInput(inputStr, expectedType = undefined) {
  ...
  if (expectedType != null) {
    ...
    throw new Error(`Invalid ${typeName} input: ${describeValue(inputStr)}`);
  }
  throw new Error('Invalid input: ' + describeValue(inputStr));
}
...
function validateSourceInput(input) {
  ...
  if (!sourceDesc || sourceDesc.indexed !== indexed) {
    throw new Error('Invalid input source: ' + describeValue(source));
  }
}
...
function validateDeviceInput(input) {
  ...
  if (!isPort(port)) {
    throw new Error('Invalid input port: ' + describeValue(port));
  }
  if (!isDevice(device)) {
    throw new Error('Invalid input device: ' + describeValue(device));
  }
}
