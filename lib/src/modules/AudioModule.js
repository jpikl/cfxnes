...
const channels = Object.keys(channelAliases);
...
export default class AudioModule {
  ...
  initOptions() {
    this.options = [
      ...
      {name: 'audioChannels', get: this.getChannels, set: this.setChannels, def: arrayToProperties(channels, 1.0)},
    ];
  }
  ...
  setChannels(channels) {
    forEachProperty(channels, this.setChannelVolume, this);
  }

  getChannels() {
    return arrayToProperties(channels, this.getChannelVolume, this);
  }

  setChannelVolume(channel, volume) {
    var channelId = channelAliases[channel];
    if (channelId != null && this.getChannelVolume(channel) !== volume) {
      logger.info(`Setting audio channel "${channel}" volume to ${~~(100 * volume)}%`);
