...
function cfxnes(initOptions = {}) {
  ...
  const instance = defineProperties({}, {
    ...
    'video': buildProp.nested({
      ...
      'clear': buildProp.method(video.clearFrame, video),
    }),
...
try {
  defineProperties(cfxnes, {'name': buildProp.constant(NAME)});
} catch (error) {
...
defineProperties(cfxnes, {
  ...
  'logLevel': buildProp.writable(log.getLevel, log.setLevel, log),
